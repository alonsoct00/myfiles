...
interjetApp.controller('StationsController', ["$scope", function($scope,  $http) {
  ...
  $('button.btn-select').on('click', function (e) {
	...
	var counter = 0;
	var RowsByColumn = 12;
	var IsThereForInterlineStations = false;
	if(Interjet.StationsAndMarketsInfo.Stations != undefined){
		$.each(Interjet.StationsAndMarketsInfo.Stations , function( index, value ) {
			if(value.ForInterline== "True"){
				IsThereForInterlineStations = true;
			}
		});

		if(IsThereForInterlineStations==true){
			RowsByColumn=16;
		}else{
			$('.stations-note').css("display","none");			
		}
	}
	...
	$.each(Interjet.StationsAndMarketsInfo.Zones , function( ZoneIndex, value ) {
			...
			$.each(Interjet.StationsAndMarketsInfo.Stations , function( index, value ) {
				if(value.Zone==ZoneIndex){					
					var station = {Code:value.Code, Name:value.Name, Interline:value.ForInterline,  CustomStyle:value.CustomStyle, type:"Station"}
					StationsNA.push(station);    								
				}	   
			});
	});		

	$scope.StationsNA = chunk(StationsNA, RowsByColumn);	
	
	
	$scope.ShowDestinationsModal = function (ForInterline, OriginStationCode, OriginStationName, $event){	
		...
		$.each(Interjet.StationsAndMarketsInfo.Zones , function( ZoneIndex, value ) {			
				...
				$.each(Interjet.StationsAndMarketsInfo.Stations , function( index, value ) {
					...
					if(value.Zone==ZoneIndex){	
						...
						var station = {Code:value.Code, Name:value.Name,  Interline:value.ForInterline, CustomStyle:value.CustomStyle, type:"Station", StatusDisplay: StatusDisplay}
		...
		$scope.DestinationsNA = chunk(DestinationsNA, RowsByColumn);			
	...
	$scope.CloseDestinationsModal = function (ForInterline, DestinationStationCode, DestinationStationName, event){					
...
$(function(){
	...
	INTERJET.SearchAvailability.RedirectToPage = function(){
		var IsInterlineMarket = false;
		var DepartureStation = $('input[id*="TextBoxMarketOrigin1"]').val();
		var ArrivalStation = $('input[id*="TextBoxMarketDestination1"]').val();	

		var Market= DepartureStation + '-'+ArrivalStation;
		console.log(Interjet.StationsAndMarketsInfo.Stations[DepartureStation].ForInterline);
		console.log(Interjet.StationsAndMarketsInfo.Stations[ArrivalStation].ForInterline);
		if(Interjet.StationsAndMarketsInfo.Stations[DepartureStation].ForInterline=="True" || Interjet.StationsAndMarketsInfo.Stations[ArrivalStation].ForInterline=="True"){
			IsInterlineMarket = true;
		}
		var FlightType = $('input[id*="_RoundTrip"]:checked').val();
		console.log(IsInterlineMarket);
		
		if(IsInterlineMarket){
			
			 var DepartureDate = $('select[id*="DropDownListMarketMonth1"]').val() + "-" + $('select[id*="DropDownListMarketDay1"]').val();	
			 
			var ReturnDate='';
			var ReturnDateString='';
			 if(FlightType=='RoundTrip'){
				ReturnDate = $('select[id*="DropDownListMarketMonth2"]').val() + "-" + $('select[id*="DropDownListMarketDay2"]').val();			 	 
				ReturnDateString ='&RD={ReturnDate}';
			 }
			 
			 			 
			 var TotalADT = $('select[id*="DropDownListPassengerType_ADT"]').val();
			 var TotalCHD = $('select[id*="DropDownListPassengerType_CHD"]').val();
			 var TotalINF = $('select[id*="DropDownListPassengerType_INFANT"]').val();
		 
			 var InterlineURL =  'https://interline.interjet.com.mx/flight/search?O={DepartureStation}&D={ArrivalStation}&DD={DepartureDate}'+ReturnDateString+'&ADT={TotalADT}&CHD={TotalCHD}&INF={TotalINF}&EC=true&Culture='+Interjet.CultureCode;
			 InterlineURL = InterlineURL.replace('{DepartureStation}',DepartureStation);
			 InterlineURL = InterlineURL.replace('{ArrivalStation}',ArrivalStation);			 
			 InterlineURL = InterlineURL.replace('{DepartureDate}',DepartureDate);			 
			 InterlineURL = InterlineURL.replace('{ReturnDate}',ReturnDate);
			 InterlineURL = InterlineURL.replace('{TotalADT}',TotalADT);
			 InterlineURL = InterlineURL.replace('{TotalCHD}',TotalCHD);
			 InterlineURL = InterlineURL.replace('{TotalINF}',TotalINF);			 
	...
	*/
	...
	$('.bookingInterjet input[id*="ButtonSubmit"]').click(function(){
		var UrlToRedirect = INTERJET.SearchAvailability.RedirectToPage();
		if(UrlToRedirect!=''){
