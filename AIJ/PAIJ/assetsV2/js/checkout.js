// /CUSTOM HUGO CHANGES/////////////
function DoPostBackForSort (marketIndex, target, argument) {
  $('#AuxSortJourneyBy' + marketIndex).val(argument)
  __doPostBack(target, argument)
}

/*function AddMonths(InputDate, Number) {
   return new Dat
   e(InputDate.setMonth(InputDate.getMonth() + Number));      
}*/
...
$(document).ready(function () {
    ...
    var lh1 = $('.tip-light').find('tr:eq(0)').height(),
        lh2 = $('.tip-light').find('tr:eq(1)').height(),
        lh3 = $('.tip-light').find('tr:eq(2)').height(),
        lh4 = $('.tip-light').find('tr:eq(3)').height(),
        lh5 = $('.tip-light').find('tr:eq(4)').height(),
        lh6 = $('.tip-light').find('tr:eq(5)').height(),
        oh1 = $('.tip-optima').find('tr:eq(0)').height(),
        oh2 = $('.tip-optima').find('tr:eq(1)').height(),
        oh3 = $('.tip-optima').find('tr:eq(2)').height(),
        oh4 = $('.tip-optima').find('tr:eq(3)').height(),
        oh5 = $('.tip-optima').find('tr:eq(4)').height(),
        oh6 = $('.tip-optima').find('tr:eq(5)').height(),
        ph1 = $('.tip-priority').find('tr:eq(0)').height(),
        ph2 = $('.tip-priority').find('tr:eq(1)').height(),
        ph3 = $('.tip-priority').find('tr:eq(2)').height(),
        ph4 = $('.tip-priority').find('tr:eq(3)').height(),
        ph5 = $('.tip-priority').find('tr:eq(4)').height(),
        ph6 = $('.tip-priority').find('tr:eq(5)').height(),

        bh1 = $('.tip-plus').find('tr:eq(0)').height(),
        bh2 = $('.tip-plus').find('tr:eq(1)').height(),
        bh3 = $('.tip-plus').find('tr:eq(2)').height(),
        bh4 = $('.tip-plus').find('tr:eq(3)').height(),
        bh5 = $('.tip-plus').find('tr:eq(4)').height(),
        bh6 = $('.tip-plus').find('tr:eq(5)').height(),
        ...
        if(bh1 > 0){
            fh1 = Math.max(lh1, oh1, ph1, bh1);
            fh2 = Math.max(lh2, oh2, ph2, bh2);
            fh3 = Math.max(lh3, oh3, ph3, bh3);
            fh4 = Math.max(lh4, oh4, ph4, bh4);
            fh5 = Math.max(lh5, oh5, ph5, bh5);
            fh6 = Math.max(lh6, oh6, ph6, bh6);
        }
        else{
            fh1 = Math.max(lh1, oh1, ph1);
            fh2 = Math.max(lh2, oh2, ph2);
            fh3 = Math.max(lh3, oh3, ph3);
            fh4 = Math.max(lh4, oh4, ph4);
            fh5 = Math.max(lh5, oh5, ph5);
            fh6 = Math.max(lh6, oh6, ph6);
        }
    var MonthIndex = 1;
 ...
 $('.flight_schedule .price-flight').on('click', function(e){

      if($(this).find('input[type="radio"]').length > 0){
        var ClickedElement = $(this).parents('.flight_schedule').attr("id");        
        document.getElementById($(this).find('input[type="radio"]').attr("id")).click();
        $(this).parents('.market-container').find('.price-flight').removeClass("flight-select");  

        $(this).parents('.market-container').find('.schedule-container').each(function (key, value) {   
            if($(this).attr("id")!= ClickedElement){
              $(this).removeClass("more-flights");
              $(this).hide();
            }
        })
        $(this).parents('.market-container').find('.showAll').show();
        $(this).addClass("flight-select")
        var market = $(this).find('input[type="radio"]').attr('market');
        var product = $(this).find('input[type="radio"]').attr('product');
        
          var charge = $(this).find('input[type="radio"]').attr('charge');
          var charge2 = $(this).next().find('input[type="radio"]').attr('charge');
          var dif = charge2 - charge;
          
          $('.priority-wrp'+'.'+market+' .btn.btn-outline').on('click', function (e) {
            e.preventDefault()
            $('.priority-wrp'+'.'+market).stop().slideUp()
          })
          var that = $(this).attr("id");
          $('.priority-wrp'+'.'+market+' .btn.btn-secondary.btn-lightblue-interjet').on('click', function (e) {
            e.preventDefault()
            e.stopPropagation()
            
              if(!isUpsell && market==0){
                isUpsell = true;
                $(this).parents('.market-container').find('.price-flight.flight-select').next().click();
                $('.priority-wrp'+'.'+market).stop().slideUp()
              }else if(!isUpsell2 && market==1){
                isUpsell2 = true;
                $(this).parents('.market-container').find('.price-flight.flight-select').next().click();
                $('.priority-wrp'+'.'+market).stop().slideUp()
              }            
          ...
          ;
          if(!isNaN(dif) && (!isUpsell || !isUpsell2)){            
            $('.priority-wrp'+'.'+market).stop().slideDown()
            var auxFormatedPrice  =SKYSALES.Util.convertToLocaleCurrency(dif.toFixed());
            //var FormatedPrice = auxFormatedPrice.substring(0,auxFormatedPrice.length-3)
            $('.priority-wrp'+'.'+market).find('#priceupsell').text(auxFormatedPrice);
            $('.priority-wrp'+'.'+market).find('#currencyupsell').text(ITJCalendars.CurrencyCode);
            $('.priority-wrp'+'.'+market).find('#priceupsellm').text('$'+dif.toFixed()+" "+ITJCalendars.CurrencyCode);            
            if(product=='Tarifai'){
              $('.priority-wrp'+'.'+market).find('#productupsell').text('Óptima');
              $('.priority-wrp'+'.'+market).find('#productupsellm').text('Óptima');
              $('.priority-wrp'+'.'+market+' .row.light').removeClass('hidden');
              $('.priority-wrp'+'.'+market+' .row.priority').addClass('hidden');
              $('.priority-wrp-mobile table'+' .light-gray').removeClass('hidden');
              $('.priority-wrp-mobile table'+' .gray').removeClass('hidden');
              $('.priority-wrp-mobile table'+' .dark-blue').addClass('hidden');
            }else if(product=='Optima'){
              $('.priority-wrp'+'.'+market).find('#productupsell').text('Priority');
              $('.priority-wrp'+'.'+market).find('#productupsellm').text('Priority');
              $('.priority-wrp'+'.'+market+' .row.light').addClass('hidden');
              $('.priority-wrp'+'.'+market+' .row.priority').removeClass('hidden');
              $('.priority-wrp-mobile table'+' .light-gray').addClass('hidden');
              $('.priority-wrp-mobile table'+' .gray').addClass('hidden');
              $('.priority-wrp-mobile table'+' .dark-blue').removeClass('hidden');
            }
          }else{
            $('.priority-wrp'+'.'+market).stop().slideUp()            
          }
  ...
  $('.sort-and-connectings input[type=checkbox]').click(function(){   
    if($(this).is(':checked')==true){
      $(this).closest('.market-container').find('.total-segments-2, .total-segments-3').css("display","none")
    }else{
      $(this).closest('.market-container').find('.total-segments-2, .total-segments-3').css("display","flex")
    }   
  })  

  $('#ChangeDateClasicCalendar').on('click', function(e){
      var SelectedDateTD = $('#Calendar_Clasic').find('.ui-datepicker-current-day').attr("id");
      if(SelectedDateTD != undefined){
          var SelectedDateText = $('#Calendar_Clasic').find('.ui-datepicker-current-day').attr("value");
          var SelectedDate = new Date(SelectedDateText);          
          
          var SelectedMonth = $('#Calendar_Clasic').find('.ui-datepicker-current-day').attr("data-year") + '-' + $('#Calendar_Clasic').find('.ui-datepicker-current-day').attr("data-month");
          var SelectedDay = $('#Calendar_Clasic').find('.ui-datepicker-current-day').attr("data-day");                    
          var DropDownSelectorDay = "DropDownListMarketDay"+Number(Number(ITJCalendars.MarketIndex)+1);          
          var DropDownSelectorMonth = "DropDownListMarketMonth"+Number(Number(ITJCalendars.MarketIndex)+1);          
          
          var ExistItem = IsItemInDropDown(Number(SelectedDay),ITJCalendars.MarketIndex)                  
          if(ExistItem){              
                $('select[id*="'+DropDownSelectorDay+'"]').val(Number(SelectedDay));
          }else{
             $('select[id*="'+DropDownSelectorDay+'"]').append( new Option('',Number(SelectedDay), true,true));
          }            

          $('select[id*="'+DropDownSelectorMonth+'"]').val(SelectedMonth);
                       

         $('#bookingCheckout input[id*="ButtonSubmit"]').click();
      }
  })
      
  $('#ChangeDateGraphCalendar').on('click', function(e){
      var SelectedDateDIV = $('#Calendar_Graphs').find('.sc-item.selected').attr("id")
      if(SelectedDateDIV != undefined){
          var SelectedDateText = $('#Calendar_Graphs').find('.sc-item.selected').attr("value")
          var SelectedDate = new Date(SelectedDateText);          
          
          var SelectedMonth = $('#Calendar_Graphs').find('.sc-item.selected').attr("data-year") + '-' + $('#Calendar_Graphs').find('.sc-item.selected').attr("data-month");
          var SelectedDay = $('#Calendar_Graphs').find('.sc-item.selected').attr("data-day");                    
          var DropDownSelectorDay = "DropDownListMarketDay"+Number(Number(ITJCalendars.MarketIndex)+1);          
          var DropDownSelectorMonth = "DropDownListMarketMonth"+Number(Number(ITJCalendars.MarketIndex)+1);          
...
$('.open-calendar, .ico-calendar').on('click', function(e){

       var TriggerElementId = $(this).attr("id");
       
       if(TriggerElementId.indexOf("StartLinkClasic") !== -1){        
          var MarketIndex = Number($(this).attr("market-index"));        
          var DeptStationDesc = $(this).attr("dept-station-desc");
          var DeptStationCode = $(this).attr("dept-station-code");
          var ArrvStationDesc = $(this).attr("arrv-station-desc");
          var ArrvStationCode = $(this).attr("arrv-station-code");
          var BeginDate = $(this).attr("selected-date");

      }else{
          var MarketIndex = ITJCalendars.MarketIndex;
          var DeptStationCode = ITJCalendars.DeptStationCode;
          var DeptStationDesc = ITJCalendars.DeptStationDesc;
          var ArrvStationCode = ITJCalendars.ArrvStationCode;
          var ArrvStationDesc = ITJCalendars.ArrvStationDesc;
          var BeginDate = ITJCalendars.SelectedDate;
      }

          $('clasicdeptstation').html(DeptStationDesc);
          $('clasicarrvstation').html(ArrvStationDesc);        

        setValuesModalCalendarClasic(MarketIndex,BeginDate, DeptStationCode,DeptStationDesc, ArrvStationCode, ArrvStationDesc);        
        SearchCalendarSelectClasic(MarketIndex, BeginDate, DeptStationCode, ArrvStationCode);

       
        $('#Modal_Clasic').modal('show');

        e.preventDefault(); 

});

$('#modalCalendarSelect').on('hidden', function () {
    MonthIndex = 1; 
});



    $('.open-graphs, .ico_graph').click(function (e) {  
       var TriggerElementId = $(this).attr("id");

       
       if(TriggerElementId.indexOf("StartLinkGraphs") !== -1){
        ...
        $('graphdeptstation').html(DeptStationDesc);
        $('grapharrvstation').html(ArrvStationDesc);        

        setValuesModalCalendarGraph(MarketIndex,BeginDate, DeptStationCode,DeptStationDesc, ArrvStationCode, ArrvStationDesc);        
        SearchCalendarSelectGraphs(MarketIndex, BeginDate, DeptStationCode, ArrvStationCode);

        $('#prev_week').css("display", "none");
        $('#next_week').css("display", "block");
        $('.sc-item').removeClass('selected');
        
        $('#Modal_Graphs').modal('show');

        e.preventDefault(); 
    });


    function setValuesModalCalendarGraph(MarketIndex, BeginDate, DepartureStation,DepartureStationDesc, ArrivalStation, ArrivalStationDesc){  
        $('.calendar-title .flight-type').hide();   
        $('#GraphModalFlightTitle'+Number(MarketIndex+1)).show(); 

        ITJCalendars.SelectedDate = BeginDate;
        ITJCalendars.DeptStationCode = DepartureStation;
        ITJCalendars.DeptStationDesc = DepartureStationDesc;
        ITJCalendars.ArrvStationCode = ArrivalStation;
        ITJCalendars.ArrvStationDesc = ArrivalStationDesc;
        ITJCalendars.MarketIndex = Number(MarketIndex);        
    }

    function setValuesModalCalendarClasic(MarketIndex, BeginDate, DepartureStation, DepartureStationDesc, ArrivalStation, ArrivalStationDesc){
  ...
  resizeColumsPrice()
  resizeMenuHover() /*Comment this line, homeCMS*/
  $('.tip-light, .tip-optima, .tip-priority, .tip-detail, .tip-plus').hide()
  ...
  $('.purchase').on('click', function (e) {
    e.preventDefault()
    $('.purchase_detail').stop().slideToggle()
    $(this).toggleClass('open')
  // se ciera lo que esté abierto
  })
...
(
  function(e)
  {
    if(e.target.className !== "purchase_detail")
    {
      $(".purchase_detail").hide();
    }
  }
);
  ...
  $('.edit-booking').on('click', function (e) {
    ...
    $($('.label-origin').parent()).click()
    $('#bookingCheckout').show()
  })

  $('.booking-bg').on('click', function (e) {
    $('#bookingCheckout').hide()
  })
  ...
  // Version Hover
  /*$('.purchase').on('mouseenter', function(e){
      e.preventDefault()
      $('.purchase_detail').stop().slideDown()
      $(this).addClass('open')
  })
  .on('click', function(e){
      e.preventDefault()
  })
  $('.purchase_detail').on('mouseleave', function(e){
      e.preventDefault()
      $('.purchase_detail').stop().slideUp()
      $('.purchase').removeClass('open')
  });*/
  ...
  $('.open-calendar, .ico-calendar').on('click', function (e) {
    e.preventDefault()
    $('.modalGraphs').modal('hide')
    $('.modalCalendar').modal('show')
  })

  // Modal Graphs
  $('.open-graphs, .ico-graph').on('click', function (e) {
    e.preventDefault()
  ...
  /*if(width < 768){
      $('.PriceDetail').on('click', function(e){
          e.preventDefault()
          $('.modalPricesMobile').modal('show')
      })
  }
  else{*/
  $('.PriceDetail').on('click', function (e) {
  ...
  // Modal Reglas Tarifarias Light
  $('.ReglasLight').on('click', function (e) {
    e.preventDefault()
    $('#ReglasLight').modal('show')
  })

  // Modal Reglas Tarifarias Optima
  $('.ReglasOptima').on('click', function (e) {
    e.preventDefault()
    $('#ReglasOptima').modal('show')
  })

  // Modal Reglas Tarifarias Priority
  $('.ReglasPriority').on('click', function (e) {
    e.preventDefault()
    $('#ReglasPriority').modal('show')
  })

  // Modal Steps
  $('.pickFlight-mob').on('click', function (e) {
    e.preventDefault()
    $('.modalSteps').modal('show')
  })
  $('.steps').picker({
    data: ['<span>1/6</span>Vuelo', '<span>2/6</span>Pasajeros', '<span>3/6</span>Asientos', '<span>4/6</span>Extras', '<span>5/6</span>Pago', '<span>6/6</span>Disfruta'],
    lineHeight: 30,
    selected: 0
  })
  ...
  if (width < 992) {
    $('.view-conn-detail').on('click', function (e) {
      e.preventDefault()
    ...
    $('.view-conn-detail').on('click', function (e) {
      e.preventDefault()
      //$(this).parents().find('.connection-detail').stop().slideDown().removeClass('hidden')
      $(this).parents('.flight_schedule').find('.connection-detail').stop().slideDown().removeClass('hidden') /*fix homeCMS*/
      $(this).next('.closeDetail').show().css({display: 'block'})
      $(this).hide()
    })
    $('.closeDetail').on('click', function (e) {
      e.preventDefault()
      //$(this).parents().find('.connection-detail').stop().slideUp().removeClass('hidden') 
      $(this).parents('.flight_schedule').find('.connection-detail').stop().slideUp().removeClass('hidden') /*fix homeCMS*/
      $(this).prev('.view-conn-detail').show()
      $(this).hide()
    })
  }

  // Show All Flights
  $('.showAllOrigin').on('click', function (e) {    
    e.preventDefault()
    $('.outbound').find('.flight_schedule').stop().slideDown().removeClass('hidden').addClass('more-flights')
    $(this).hide()
  })

  $('.showAllDestination').on('click', function (e) {    
    e.preventDefault()
    $('.inbound').find('.flight_schedule').stop().slideDown().removeClass('hidden').addClass('more-flights')
    $(this).hide()
  })

  // Close Priority
  $('.priority-wrp a.close-btn').on('click', function (e) {
    e.preventDefault()
    $('.priority-wrp').stop().slideUp()
  })

  // Footer
  if (width < 992) {
    $('.atn-footer').hide()
  // $('.atn-mobile').addClass('open')
  }
  ...
  // Charts
  var labels = ['Dom<br/>17', 'Lun<br/>18', 'Mar<br/>19', 'Mié<br/>20', 'Jue<br/>21', 'Vie<br/>22', 'Sab<br/>23', 'Dom<br/>24', 'Lun<br/>25', 'Mar<br/>26', 'Mié<br/>27', 'Jue<br/>28', 'Vie<br/>29', 'Sab<br/>30', 'Dom<br/>31']
  var values = [900, 1200, 800, 900, 1000, 900, 800, 900, 1200, 900, 1200, 800, 900, 1000, 900]
  var outputValues = [900, '1,200', 800, 900, '1,000', 900, 800, 900, '1,200', 900, '1,200', 800, 900, '1,000', 900]
  var labelsMob = ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sab']
  ...
  function resizeMenuHover () {
    // console.log("MenuOver!")
    var flightOriginWidth = $('.flight-origin').width()
    var flightDestinationWidth = $('.flight-destination').width()
    var flightPassengersWidth = $('.flight-passengers').width() + 1
    var flightOriginOffset = $('.flight-origin').offset()
    //console.log(flightOriginOffset)
    //console.log(flightOriginOffset.left)
    var flightDestinationOffset = $('.flight-destination').offset()
    var flightPassengersOffset = $('.flight-passengers').offset()
    $('.edit').width(flightOriginWidth + flightDestinationWidth).css({left: flightOriginOffset.left})
    $('.editPassengers').width(flightPassengersWidth).css({left: flightPassengersOffset.left})
  }


/*Fix UX on hover Edit behavior*/
  /*if ($(window).width() > 1366) {
  $('.edit').width(flightOriginWidth + flightDestinationWidth).css({left: flightOriginOffset.left-200})
  $('.editPassengers').width(flightPassengersWidth).css({left: flightPassengersOffset.left-200})
}
else {
  ...
  var timerVuelos, timerPasajeros
  // Hover de vuelos}


  // columnas de tarifas
    function resizeColumsPrice(){
        //heightAuto();

       // $('.tip-light, .tip-optima, .tip-priority, .tip-detail, .tip-plus').hide();

        var btnLightPosition = $('.fhg-light.active').offset();
        var btnLightWidth = $('.fhg-light').width() + 41;
        var btnLightHeight = $('.fhg-light').height() + 35;

        //console.log( 'resizeColumsPrice' + btnLightPosition, $('.fhg-light.active'));
        var btnOptimaPosition = $('.fhg-optima.active').offset();
        var btnOptimaWidth = $('.fhg-optima').width() + 41;
        var btnOptimaHeight = $('.fhg-optima').height() + 35;

        var btnPriorityPosition = $('.fhg-priority.active').offset();
        var btnPriorityWidth = $('.fhg-priority').width() + 41;
        var btnPriorityHeight = $('.fhg-priority').height() + 35;

        var btnPlusPosition = $('.fhg-plus.active').offset();
        var btnPlusWidth = $('.fhg-plus').width() + 41;
        var btnPlusHeight = $('.fhg-plus').height() + 35;

        var detailWidth = $('.tip-detail').width();

        if(btnLightPosition != undefined)
        {
            $('.tip-light').width(btnLightWidth).css({
                left: btnLightPosition.left - 5,
                top: btnLightPosition.top + btnLightHeight
            }).find('td').width(btnLightWidth);

            $('.tip-optima').width(btnOptimaWidth).css({
                left: btnOptimaPosition.left - 5,
                top: btnOptimaPosition.top + btnOptimaHeight
            }).find('td').width(btnOptimaWidth);

            $('.tip-priority').width(btnPriorityWidth).css({
                left: btnPriorityPosition.left - 5,
                top: btnPriorityPosition.top + btnPriorityHeight
            }).find('td').width(btnPriorityWidth);

            if(btnPlusPosition != undefined){
                $('.tip-plus').width(btnPlusWidth).css({
                    left: btnPlusPosition.left - 5,
                    top: btnPlusPosition.top + btnPlusHeight
                }).find('td').width(btnPlusWidth);
            }


            $('.tip-detail').css({
                left: btnLightPosition.left - detailWidth - 5,
                top: btnLightPosition.top + btnLightHeight
            });
        }
    }
  ...
  if (width > 768) {
    ...
    $('.outbound .tags .fhg-light img, .outbound .tags .fhg-optima img, .outbound .tags .fhg-priority img , .outbound .tags .fhg-plus img, .inbound .tags .fhg-light img, .inbound .tags .fhg-optima img, .inbound .tags .fhg-priority img, .inbound .tags .fhg-plus img').on('mouseenter', function (e) {
      e.preventDefault()

      $('.outbound .tags .fhg-light, .outbound .tags .fhg-optima, .outbound .tags .fhg-priority, .outbound .tags .fhg-plus, .inbound .tags .fhg-light, .inbound .tags .fhg-optima, .inbound .tags .fhg-priority, .inbound .tags .fhg-plus').removeClass('active')

      $(this).parents('.tags').find('.fhg-optima, .fhg-priority, .fhg-light, .fhg-plus').addClass('active')
      resizeColumsPrice()
      //$('.tip-detail').addClass('openTooltip')
      $('.tip-light, .tip-optima, .tip-priority, .tip-detail, .tip-plus').stop().slideToggle('fast')

    // elemMaxHeight( $( 'table.fhg-tooltip td' ) )
    // elemMaxHeight( $( 'table.fhg-tooltip' ) )
    // , .inbound .fhg-light, .inbound .fhg-optima
    })


    $('.outbound .tags .fhg-light, .outbound .tags .fhg-optima, .outbound .tags .fhg-priority, .outbound .tags .fhg-plus, .inbound .tags .fhg-light, .inbound .tags .fhg-optima, .inbound .tags .fhg-priority, .inbound .tags .fhg-plus').on('click', function (e) {
      e.preventDefault()

      $('.outbound .tags .fhg-light, .outbound .tags .fhg-optima, .outbound .tags .fhg-priority, .outbound .tags .fhg-plus, .inbound .tags .fhg-light, .inbound .tags .fhg-optima, .inbound .tags .fhg-priority, .inbound .tags .fhg-plus').removeClass('active')

      $(this).addClass('active').parent().find('.fhg-optima, .fhg-priority, .fhg-light, .fhg-plus').addClass('active')
      resizeColumsPrice()
      //$('.tip-detail').addClass('openTooltip')
      $('.tip-light, .tip-optima, .tip-priority, .tip-detail, .tip-plus').stop().slideToggle('fast')
    ...
    $('.outbound .fhg-light, .outbound .fhg-optima, .outbound .fhg-priority, .outbound .fhg-plus, .tip-light, .tip-optima, .tip-priority, .tip-plus, .tip-detail, .inbound .fhg-light, .inbound .fhg-optima, .inbound .fhg-priority, .inbound .fhg-plus')
      .on('mouseleave', function (e) {
        e.preventDefault()
        timerColumns = setTimeout(function () {
          $('.tip-light, .tip-optima, .tip-priority, .tip-detail, .tip-plus').stop().slideUp('fast')
          $('.tip-detail').removeClass('openTooltip')
          $('.outbound .fhg-light, .outbound .fhg-optima, .outbound .fhg-priority, .outbound .fhg-plus, .inbound .fhg-light, .inbound .fhg-optima, .inbound .fhg-priority, .inbound .fhg-plus').removeClass('active')
        }, 1)
      })
  }

  // Scroll to PriorityMobile
  $('.BtnPriorityMobile').on('click', function (e) {
    var PriorityMobile = $('.priority-wrp-mobile').offset()
    e.preventDefault()
    // $( '.priority-wrp-mobile' ).show()
    $('html, body').animate({scrollTop: PriorityMobile.top - 42 }, 1000)
    return false
  })

  /*$('.edit.flight-origin').css('left', flightOrigin.left)
  $('.edit.flight-destination').css('left', flightDestination.left)
  $('.edit.flight-passengers').css('left', flightPassengers.left);*/
  ...
  function heightAuto(){
    ...
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(0)').find('td').removeAttr("style").height(fh1-20);
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(1)').find('td').removeAttr("style").height(fh2-20);
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(2)').find('td').removeAttr("style").height(fh3-20);
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(3)').find('td').removeAttr("style").height(fh4-20);
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(4)').find('td').removeAttr("style").height(fh5-20);
    $('.tip-detail, .tip-light, .tip-optima, .tip-priority, .tip-plus').find('tr:eq(5)').find('td').removeAttr("style").height(fh6-20);
  }

  function liquid () {
    var height = window.innerHeight ? window.innerHeight : $(window).height()
    $('.purchase_detail').css({ 'max-height': height - $('.menuScheduleWrp').outerHeight() })
    $('table.fhg-tooltip td').height('40')
   heightAuto();

    if (height < 660) {
      $('.modalGraphs .modal-dialog').css({ 'max-height': height * .96, 'overflow': 'auto', '-webkit-overflow-scrolling': 'touch' })
    }
  ...
  // ///// CONTACT CUSTOM SCRIPTS JEMV 080218 //////////

  // despliega listado de programas de recompensas
  $('.btn-programa-recompensas').on('click', function (e) {
    var currentButton = $(this)[0].id
    var indx = $(this)[0].id.split('_')[1]
    var currentSubMenu = 'dvRecompensas_' + indx
  ...
  // muestra/oculta el campo 'nÃƒÂºmero de socio'
  $('.opcion-programaRecompensas').on('click', function (e) {
    var currentOption = $(this)[0].id
    var indx = currentOption.split('_')[1]

    var thisDropdown = $('#btnProgramaRecompensas_' + indx)
    var arrayOfSpan = thisDropdown.find('span')

    $(arrayOfSpan[arrayOfSpan.length - 1]).text($(this).text())
    $('#dvRecompensas_' + indx).slideToggle('fast')

    if (program.split('_')[0] == 'NO') {
      $('.form-num-socio_' + indx).hide()
      $('.btn-wrp_' + indx).hide()
      $('.customerIDbox_' + indx).val('')
    }else {
      $('.form-num-socio_' + indx).show()
      $('.btn-wrp_' + indx).show()
    }
  })

  // agrega funcionalidad a cada botÃƒÂ³n que valida el nÃƒÂºmero de socio
  $('.btn-check').on('click', function (e) {
    var mmbrNmbr = $('.customerIDbox_' + indx).val()
    validateMembershipNumber()
  })

  // valida que exista el nÃƒÂºmero de socio ingresado
  function validateMembershipNumber () {
    var Current_program_request = program
    var Current_pax = indx
    var Customer_id = $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).val()

    if (isPostBack != null) {
      if (isPostBack == false) {
        $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().removeClass('correct')
        $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().removeClass('incorrect')
      }
    }else {
      $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().removeClass('correct')
      $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().removeClass('incorrect')
    ...
    if (Current_program_request == 'CIJ') {
      if (Customer_id != '') {
        // open Modal
        var modalToDisplay = '#Modal_LoadingPersonInfo'
        modalToDisplay = '#loader'
        $(modalToDisplay).modal('show')

        // reset instances
        $('#SearchPersonProgressBar').removeClass('hidden')
        $('#AJAXSearchPersonResponseContainer').html('')
        $('#SearchPersonResponse').addClass('hidden')
        $('#addFamily_LoadingPersonInfo').addClass('hidden')

        jQuery.ajax({
          type: 'POST',
          url: '../GetPersonInfo.aspx',
          data: {Passenger: Current_pax, CustomerNumber: Customer_id},
          success: function (result) {
            jQuery('#AJAXSearchPersonResponseContainer').html(result)

            if (result.indexOf('FamilyMembership') != -1) {
              $('#AJAXSearchPersonResponseContainer').html(result)
              $('#Modal_LoadingPersonInfo').modal('show')
              $('#addFamily_LoadingPersonInfo').removeClass('hidden')
            }

            if (result.indexOf('LoadInformationToPax') != -1) {
              $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().addClass('correct')
            }else {
              $('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().addClass('incorrect')
            }
            $('#loader').modal('hide')
          }
        })
      ...
      if (Current_program_request != 'NO') 
	  {
		var AliasNumber = '308681' + Customer_id.replace('308681', '')

		if (isCardNumberValid(AliasNumber)) {
			__doPostBack('PaybackRequestADD', AliasNumber + '|' + Current_pax)
			$('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().addClass('correct')
			document.getElementById('btnProgramaRecompensas_' + indx).disabled=true;
		}else {
			// alert('El nÃƒÂºmero Payback que estas ingresando no es vÃƒÂ¡lido')
			$('#CONTROLGROUPNewContactViewA_PassengerInputViewNewContactViewA_TextBoxCustomerNumber_' + indx).parent().parent().addClass('incorrect')
		}
	  }
    }
  }
  // verifica que sea vÃƒÂ¡lido el nÃƒÂºmero Payback ingresado
  ...
  // agrega la clase 'active' al un elemento '.form-control' ó '.cntcPhone' cuando tenga el foco
  $('.form-control').bind('focus', function () 
  {
    $(this).parent().parent().addClass('active')
	
	// si el elemento tiene la clase 'incorrect-input' el borde se mantiene en color rojo aunque tenga el foco del cursor:
	if($(this).attr('class').indexOf('incorrect-input') != -1)
	{
		$(this).parent().css('borderColor', "#f50029");
	}
  });
  $('.cntcPhone').bind('focus', function () 
  {
    $(this).parent().parent().parent().addClass('active')
  });
  // quita la clase 'active' al un elemento '.form-control' ó '.cntcPhone' cuando pierde el foco
  $('.form-control').bind('blur', function () 
  {
    $(this).parent().parent().removeClass('active')
  });
  $('.cntcPhone').bind('blur', function () 
  {
    $(this).parent().parent().parent().removeClass('active')
	
	// si el elemento tiene la clase 'incorrect-input' el borde se mantiene en color rojo al perder el foco del cursor:
	if($(this).attr('class').indexOf('incorrect-input') != -1)
	{
		$(this).parent().css('borderColor', "#f50029");
	}
...
;(function ($) {
  $.widget('jtsage.datebox', {
    ...
    _build: {
      ...
      calbox: function () {
        ...
        if (w.wheelExists) {
          ...
          w.d.intHTML.on('DOMMouseScroll', function (e, d) {
            e.preventDefault()
            if (d > 0 && w.calNext) {
              if (w.calBackDate === false) {
                w.calBackDate = new Date(w.theDate.getTime())
      ...
      datebox: function () {
        ...
        if (w.wheelExists) {
          ...
          allControls.on('DOMMouseScroll', 'input', function (e, d) {
            e.preventDefault()
            w._dbox_delta = d < 0 ? -1 : 1
            w._offset($(this).data('field'), (d < 0 ? -1 : 1) * $(this).data('amount'))
          })
      ...
      flipbox: function () {
        ...
        if (w.wheelExists) {
          ...
          w.d.intHTML.on('DOMMouseScroll', '.ui-overlay-shadow', function (e, d) {
            e.preventDefault()
            w._offset($(this).data('field'), (d < 0 ? 1 : -1) * $(this).data('amount'))
          })
        }
      ...
      slidebox: function () {
        ...
        if (w.wheelExists) {
          ...
          w.d.intHTML.on('DOMMouseScroll', '.ui-datebox-sliderow-int', function (e, d) {
            e.preventDefault()
            w._offset($(this).data('rowtype'), (d < 0 ? -1 : 1) * ($(this).data('rowtype') === 'i' ? o.minuteStep : 1))
          })
        }
    ...
    _create: function () {
      ...
      if ((typeof $.event.special.mousewheel !== 'undefined') || (typeof $.event.special.wheel !== 'undefined')) {
        w.wheelExists = true
      }
