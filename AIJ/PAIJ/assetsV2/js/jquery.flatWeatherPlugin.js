...
;(function ( $, window, document, undefined ) {
	    ...
	    var defaults = {
				...
				strings : { //strings for translation							
							days: ['Sun-Dom-Dim','Mon-Lun-Lun','Tue-Mar-Mar','Wed-Mié-Mer','Thu-Jue-Jeu','Fri-Vie-Ven-','Sat-Sáb-Sam'],
							min : "Min",
							max : "Max",
		...
		$.extend(Plugin.prototype, {
			...
			render : function (weather, context) {
				...
				if (context.settings.displayCityNameOnly) {
					$("#weather-title").text(weather.city);
				}
				else {
					$("#weather-title").text(weather.location);					
				}
				
				
				if (context.settings.view != "forecast") {
					var today = "";
					$("#weather-icon").html('<div class="wi wi'+weather.today.code+'" style="margin-left: 10px;color: #1B4298; font-size: 30px;"><div/>');					
					$("#weather-celsius").html(weather.today.temp.now + " " + degrees);					
				}
				...
				if (context.settings.view != "simple") {
					if (context.settings.view != "today" || context.settings.view == "forecast") {
						...
						for (var i = startingIndex; i < weather.forecast.length; i++) {
							//$("#weather-title-day_" + i).html(weather.forecast[i].day);
							$("#weather-title-day_" + i).val(weather.forecast[i].day);
							$("#weather-icon_" + i).html('<div class="wi wi'+weather.forecast[i].code+'" style="color: #1B4298; font-size: 27px;"><div/>');	
							$("#Weather-day_" + i).html(weather.forecast[i].temp.max + "<sup>" + degrees + "</sup> - " + weather.forecast[i].temp.min + "<sup>" + degrees + "</sup>");
						}
					}
				}
			}
