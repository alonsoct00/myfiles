	...
	$.ajax({
		...
		if(info == ""){
			...
			if(ParamHotel.Hotelid != ''){
				...
				var totalPages = $("#countPages").html();
				if(totalPages != null || totalPages > 1) mostrarPaginado(totalPages);
			}					
		}																												
...
function mostrarPaginado(countTotalPages){
	$('#pagination-demo').twbsPagination({
		...
		onPageClick: function (event, page) {
			if(ParamHotel.Hotelid == 0 && llamadoInicio == false){
				showHotelsPage(page);
				//$("#paginActual").html(page);
...
function showHotelsPage(page){
	rooms = $("#selectRooms").html();
	var data = getData(page, false, null);
	llamadoAjaxHotels(data);		
	open = true;	
}